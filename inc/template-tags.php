 ...
 * Prints HTML with meta information for the categories, tags and comments.
 */
if (!function_exists('understrap_entry_footer_home')) {
	function understrap_entry_footer_home()
	{
		// Hide category and tag text for pages.
		if ('post' === get_post_type()) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list(esc_html__(', ', 'understrap'));
			if ($categories_list && understrap_categorized_blog_home()) {
				/* translators: %s: Categories of current post */
 ...
 * @return bool
 */
if (!function_exists('understrap_categorized_blog_home')) {
	function understrap_categorized_blog_home()
	{
		if (false === ($all_the_cool_cats = get_transient('understrap_categories'))) {
			// Create an array of all the categories that are attached to posts.
			$all_the_cool_cats = get_categories(array(
